<div class="flex h-full w-full flex-1 flex-col gap-6 p-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('roompage') }}"
            class="flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
    ...
    <div class="overflow-auto rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-800">
        <form wire:submit.prevent="roomUpdate" class="space-y-6">
            ...
            <div>
                ...
                <div
                    class="mb-4 flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-6 hover:bg-gray-100 dark:border-gray-600 dark:bg-gray-900 dark:hover:bg-gray-800">
                    <label class="flex cursor-pointer flex-col items-center gap-2">
                        ...
                        <span class="text-xs text-gray-500 dark:text-gray-400">PNG, JPG up to 10MB (Multiple files
                            allowed)</span>
                        <input type="file" class="hidden" multiple accept="image/*" wire:model="newImages" />
                    </label>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    ...
                    @if ($images && count($images) > 0)
                        @foreach ($images as $index => $image)
                            <div
                                class="group relative aspect-square overflow-hidden rounded-lg border border-gray-200 bg-gray-100 dark:border-gray-700 dark:bg-gray-900">
                    ...
                    @if (!empty($newImages) && count($newImages) > 0)
                        @foreach ($newImages as $index => $newImage)
                            <div
                                class="group relative aspect-square overflow-hidden rounded-lg border border-gray-200 bg-gray-100 dark:border-gray-700 dark:bg-gray-900">
                                <img src="{{ $newImage->temporaryUrl() }}" alt="New Preview"
                                    class="h-full w-full object-cover" />
            ...
            <div class="grid gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" id="room-name" value="{{ $room->roomName }}" wire:model="roomName"
                ...
                <div>
                    ...
                    <input type="text" id="room-number" value="{{ $room->roomNumber }}" wire:model="roomNumber"
                        placeholder="e.g., 101"
                        class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400 dark:focus:ring-blue-400" />
                    @error('roomNumber')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="room-rate" value="{{ $room->roomPrice }}" wire:model="roomRate"
                ...
                <div>
                    ...
                    <input type="number" id="room-capacity" value="{{ $room->capacity }}" wire:model="roomCapacity"
                ...
                <div>
                    <label for="room-status" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">
                        Room Status <span class="text-red-500">*</span>
                    </label>
                    <select id="room-status" wire:model="roomStatus"
                        class="w-full cursor-pointer rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white dark:focus:border-blue-400 dark:focus:ring-blue-400">
            ...
            <div>
                ...
                <textarea id="room-description" wire:model="roomDescription" rows="4"
                    placeholder="Describe the room features, amenities, and highlights..."
                    class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-500 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-900 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-400 dark:focus:ring-blue-400">{{ $room->roomDescription }}</textarea>
            ...
            <div class="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                {{-- Cancel Button (Flux version) --}}
                <flux:button href="{{ route('roompage') }}" variant="subtle">
                    Cancel
                </flux:button>

                {{-- Save Button (Flux version) --}}
                <flux:button type="submit" variant="primary" loading="roomUpdate"
                    class="bg-green-600 hover:bg-green-700 dark:bg-green-500 dark:hover:bg-green-600 border-none text-white">
                    Save Room
                </flux:button>
            </div>
        </form>
