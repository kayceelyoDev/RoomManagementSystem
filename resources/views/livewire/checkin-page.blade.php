<div class="min-h-screen bg-gray-100 dark:bg-gray-900 p-6 transition-colors duration-300">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8">
        ...
        <div class="flex gap-3 mt-4 md:mt-0">
            ...
            <button class="flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div wire:click="selectReservation({{ $res['id'] }})" 
            ...
            <div class="absolute left-0 top-0 bottom-0 w-1.5 {{ $res['is_room_ready'] ? 'bg-green-500' : 'bg-orange-400' }}"></div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center pl-3">
                <span class="text-xs font-medium {{ $res['is_room_ready'] ? 'text-green-600 dark:text-green-400' : 'text-orange-500 dark:text-orange-400' }}">
                    {{ $res['is_room_ready'] ? '● Room Ready' : '● Housekeeping' }}
                </span>
                <span class="text-sm font-bold text-gray-900 dark:text-white">${{ number_format($res['total_amount'], 2) }}</span>
            </div>
        </div>
        @endforeach
    </div>
    ...
    <div wire:transition.opacity 
        ...
        <div wire:transition.scale.origin.center
            ...
            <div class="w-full md:w-1/2 p-8 border-r border-gray-200 dark:border-gray-700 overflow-y-auto
                        bg-gray-50 dark:bg-gray-900/50">
                <h2 class="text-xl font-bold text-gray-800 dark:text-white mb-6">Reservation Details</h2>
                
                <div class="flex items-center gap-4 mb-6 p-4 rounded-xl border 
                            bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700">
                ...
                <div class="grid grid-cols-2 gap-y-6 gap-x-4 mb-8 text-gray-800 dark:text-gray-200">
                    <div>
                        ...
                        <p class="font-medium">{{ $selectedReservation['check_in'] }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Check Out</label>
                        <p class="font-medium">{{ $selectedReservation['check_out'] }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Room Type</label>
                        <p class="font-medium">{{ $selectedReservation['room_type'] }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase">Assigned Room</label>
                        <div class="flex items-center gap-2">
                            <span class="font-bold text-lg text-blue-600 dark:text-blue-400">{{ $selectedReservation['room_number'] }}</span>
                            @if(!$selectedReservation['is_room_ready'])
                                <span class="text-xs text-orange-500 bg-orange-100 dark:bg-orange-900/30 px-2 py-0.5 rounded">Cleaning</span>
                            @endif
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300">Reception Notes</label>
                    <textarea wire:model="addNote" rows="3" 
                        class="w-full mt-1 rounded-lg shadow-sm text-sm transition-colors
                ...
                <div>
                    ...
                    <div class="space-y-3 mb-8">
                        <div class="flex justify-between text-gray-600 dark:text-gray-400">
                            ...
                            <span>${{ number_format($selectedReservation['total_amount'], 2) }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600 dark:text-gray-400">
                        ...
                        <div class="flex justify-between text-green-600 dark:text-green-400">
                            ...
                            <span>- ${{ number_format($selectedReservation['deposit'], 2) }}</span>
                        </div>
                        <div class="border-t border-dashed border-gray-300 dark:border-gray-600 pt-3 flex justify-between items-center">
                            ...
                            <span class="font-bold text-2xl text-blue-600 dark:text-blue-400">${{ number_format($this->balance, 2) }}</span>
                        </div>
                    </div>

                    <div class="p-4 rounded-xl border mb-6 transition-colors
                        ...
                        <div class="grid grid-cols-3 gap-2 mb-4">
                            @foreach(['cash' => 'Cash', 'card' => 'Card', 'other' => 'Other'] as $method => $label)
                        ...
                        <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase mb-2">Amount Tendered</label>
                        <div class="relative">
                        ...
                        <div class="mt-4 flex justify-between items-center px-3 py-2 rounded border
                                    bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-600">
                            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Change / Balance</span>
